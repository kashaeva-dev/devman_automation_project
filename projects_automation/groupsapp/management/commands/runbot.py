    ...
    def handle(self, *args, **options):
        ...
        def start_conversation(update, _):
            ...
            if update.effective_chat.id in project_manager_ids:
                ...
                main_menu_keyboard = [
                    [
                        InlineKeyboardButton('Создать группы',
                                             callback_data='make_groups'),
                        InlineKeyboardButton('Посмотреть группы',
                                             callback_data='look_groups'),

                    ],
                    [
                        InlineKeyboardButton(
                            'Создать тестовые слоты для студентов',
                ...
                update.message.reply_text(
                    text='Вы не являетесь менеджером проекта. '
                         'Пожалуйста, перейдите к функционалу для студентов!',
                    )
        ...
        def make_groups(update, _):
            ...
            project_manager = (Project_manager
                               .objects.prefetch_related('schedule')
                               .get(telegram_id=update.effective_chat.id))
            timeslots = Timeslot.objects.all()
            created_groups_count = 0
            for schedule in project_manager.schedule.all():
                for timeslot in timeslots:
                    if (timeslot.start_time >= schedule.start_time and
                            timeslot.end_time <= schedule.end_time):
        ...
        def look_groups(update, _):
            ...
            groups = (Group.objects.prefetch_related('students')
                      .filter(week=Week.objects.get(actual=True),
                              project_manager=project_manager,
                ...
                if group.pk not in group_ids:
                    ...
                    for student in group.students.all():
                        group_students.append(
                            f'{student.firstname} '
                            f'{student.lastname} - '
                            f'{student.get_level_display()}'
                        )
                    group_students_text = '\n'.join(group_students)
                    groups_info.append(''.join(
                        [slot, group_students_text, '\n']))
                    logger.info({''.join(
                        [slot, group_students_text, '\n'])})
                groups_info_text = '\n'.join(groups_info)
            keyboard = [
        ...
        def student_assignment(update, _):
            ...
            for intersection in intersections:
                ...
                for manager in manager_intersactions[intersection]:
                    ...
                    logger.info(f'manager: {project_manager}')
                                ...
                                while slots and slots.first().students_count > 1 and available_groups:
                                    ...
                                    for place in places:
                                        if current_group:
                                            ...
                                            logger.info(
                                                f'available_students_count: {available_students.count()}')
                                    ...
                                    logger.info(
                                        f'available_groups for step {step}: '
                                        f'{available_groups}')
                                ...
                                slots = (
                                    Timeslot.objects.annotate(
                                        students_count=Coalesce(
                                            Subquery(
                                                StudentProjectSlot.objects
                                                .filter(
                                                    student__week=week,
                                                    student__student__level=level,
                                                    slot_id=OuterRef('id')
                                                ).exclude(
                                                    student__student__in=Student.objects
                                ...
                                while slots and slots.first().students_count > 1 and available_groups:
                                    ...
                                    slots = (
                                        Timeslot.objects.annotate(
                                            students_count=Coalesce(
                                                Subquery(
                                                    StudentProjectSlot.objects.filter(
                                                        student__week=week,
                                                        student__student__level=level,
                                                        slot_id=OuterRef('id')
                                                    ).exclude(
                                                        student__student__in=Student.objects
                                                        .prefetch_related('student_group')
                                                        .filter(student_group__group__week=week)
        ...
        conv_handler = ConversationHandler(
            entry_points=[CommandHandler('start', start_conversation),
                          CallbackQueryHandler(
                              start_conversation, pattern='to_start'),
                          ],
            states={
                'MAIN_MENU': [
                    CallbackQueryHandler(
                        make_groups, pattern='make_groups'),
                    CallbackQueryHandler(
                        look_groups, pattern='look_groups'),
                    CallbackQueryHandler(
                        make_student_slots, pattern='make_student_slots'),
                    CallbackQueryHandler(
                        student_assignment, pattern='student_assignment'),
